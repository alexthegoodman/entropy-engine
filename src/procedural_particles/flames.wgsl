...
fn vs_main(input: VertexInput) -> VertexOutput {
    ...
    // let spread = vec2<f32>(0.0, 0.0);  // Perfect targeting
    ...
    // Spawn particles at source
    let spawn_pos = uniforms.position;

    // Direction towards target
    let dir_to_target = normalize(target_pos - spawn_pos);

    // Initial velocity towards target
    let speed = mix(uniforms.initial_speed_min, uniforms.initial_speed_max, rand2.x);
    let initial_vel = dir_to_target * speed;
