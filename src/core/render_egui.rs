...
use crate::procedural_particles::particle_system::{ParticleSystem, ParticleUniforms};
 ...
 pub fn render_egui(pipeline: &mut EntropyPipeline, ctx: &egui::Context) {
        ...
        egui::SidePanel::left("activity_bar")
            ...
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ...
                    // if ui.selectable_label(pipeline.show_central_chat_overlay, "⚡").on_hover_text("Toggle Central Chat Overlay").clicked() {
                    //     pipeline.show_central_chat_overlay = !pipeline.show_central_chat_overlay;
                    // }
                });
            });
        ...
        if pipeline.current_workspace == Workspace::Sophia {
            ...
            // if pipeline.current_workspace == Workspace::Stunts {
            //     egui::TopBottomPanel::bottom("video_timeline_panel")
            //         .resizable(true)
            //         .default_height(300.0)
            //         .show(ctx, |ui| {
            //             DockArea::new(&mut pipeline.video_timeline_dock_state)
            //                 .style(Style::from_egui(ctx.style().as_ref()))
            //                 .show_inside(ui, &mut viewer);
            //         });
            // }
            ...
            //     });
             ...
             // if let Some(editor) = &mut viewer.context.export_editor {
                ...
                egui::CentralPanel::default()
                    ...
                    let sidebar_width = match pipeline.current_workspace {
