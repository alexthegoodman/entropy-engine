...
Entropy.Addon.onInit(async () => {
    ...
    await Entropy.UI.createWindow({
        ...
        onRender: async () => {
            await Entropy.UI.Widget.label("Noise Parameters", { text: "Noise Parameters", bold: true });
            ...
            await Entropy.UI.Widget.label(`Current Seed: ${terrainParams.seed}`, { text: `Current Seed: ${terrainParams.seed}` });
            await Entropy.UI.Widget.label(`Mode: ${terrainParams.usePBR ? "PBR" : "Non-PBR"}`, { text: `Mode: ${terrainParams.usePBR ? "PBR" : "Non-PBR"}` });
        }
    });
});
