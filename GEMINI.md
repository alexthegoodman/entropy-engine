...
I have already taken the liberty of moving many files from the respective repos and into the entropy-engine, so we have a good start. Now we need to pull it together in a an entropy-engine/src/startup.rs file. Take inspiration from the export/pipeline.rs file.
