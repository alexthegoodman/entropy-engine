...
| `Widget.sheetGrid` | A spreadsheet grid: lettered column headers (A, B, C...), numbered rows, one selected cell with arrow-key/Tab/Enter navigation, and an optional colored border per cell. No inline editing - drive a cell's content through your own `textInput` bound to `selected`, like a real formula bar. `options.maxHeight` scrolls the rows inside a capped box with the column header fixed above it. Your addon owns the cells and any formula evaluation. |
