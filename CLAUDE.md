...
- Prior posts in this series: [2026-09-08 - Replacing egui With Our Own Immediate-Mode GUI Kit](../posts/2026-09-08-replacing-egui-with-entropy-gui.md); [2026-09-08 - FFT Ocean Water: A GPU Compute Pipeline for Entropy](../posts/2026-09-08-fft-ocean-water.md); [2026-09-08 - Interactive Ripples on Top of Entropy's FFT Ocean](../posts/2026-09-08-fft-water-interactive-ripples.md)
- Queued next: (1) Rivers variant of the FFT ocean addon - `jonswap_spectrum()` is already fully written in `fft_water_addon.ts` but dead code (commented out at the call site); wiring it in and retuning fetch/wind params is the whole task, not new architecture. (2) Buoyancy - now that there's a combined FFT+ripple height field being sampled per-vertex on the GPU, reading that same value back on the CPU/JS side for a floating object is a smaller problem than the ripple layer itself was. (3) Object-driven wakes (a moving object injecting ripple splats itself, not just the mouse) - the splat/step/render-copy plumbing from this session's ripple work is already reusable for that, it's a new caller, not new architecture.
