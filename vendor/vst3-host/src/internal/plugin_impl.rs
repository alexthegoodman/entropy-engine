    ...
    fn open_editor(&mut self, parent: *mut std::ffi::c_void) -> Result<()> {
        ...
        if let Some(ref controller) = self.controller {
            unsafe {
                ...
                // ENTROPY PATCH (see vendor/VENDORED.md): NI's Massive and Maschine 3 answer
                // getSize with an error until the view is attached. pluginterfaces documents
                // IPlugView::getSize as "the size of the platform representation of the view", and
                // `attached` is what creates that representation, so an error here is defensible.
                // Keep the default rect and let the caller re-read the real size once `attached`
                // has succeeded.
                if view.getSize(&mut view_rect) == kResultOk {
                    view_rect_size(&view_rect)?;
                } else {
                    log::warn!("plugin editor getSize failed before attach; the size is read again after");
                }
