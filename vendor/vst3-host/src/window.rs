        ...
        #[cfg(target_os = "windows")]
        {
            unsafe {
                ...
                match plugin.open_editor(window_handle) {
                    Ok(()) => {
                        // ENTROPY PATCH (see vendor/VENDORED.md): the size asked for above came from
                        // a throwaway view that may not have known it yet (Massive, Maschine 3), so
                        // ask the attached view and resize to that.
                        let attached_size = plugin.get_editor_size().ok();
                        drop(plugin);
                        self.native_window = Some(hwnd);
                        if let Some((w, h)) = attached_size {
                            if (w, h) != (width, height) {
                                self.resize_native_window(w, h);
                            }
                        }
