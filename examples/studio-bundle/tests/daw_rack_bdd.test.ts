    ...
    for (const scenario of parseFeature("daw_rack")) {
        it(scenario.name, async () => {
            ...
            const steps: [RegExp, (...m: string[]) => void | Promise<void>][] = [
                ...
                [/^the drum rack window is shown$/, () => { world.render(); expect(w.windowVisible[Object.keys(w.windowTitles).find(id => w.windowTitles[id] === "Drum Rack")!]).toBe(true); }],
                [/^the drum rack window is hidden$/, () => { world.render(); expect(w.windowVisible[Object.keys(w.windowTitles).find(id => w.windowTitles[id] === "Drum Rack")!]).toBe(false); }],
